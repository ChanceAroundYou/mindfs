...
func (h *StreamHub) SetPendingUserAt(rootID, sessionKey, sessionTitle, agent, model, modelDisplayName, mode, effort, fastService string, planMode bool, content string, timestamp time.Time) *PendingUserMessage {
	h.mu.Lock()
	defer h.mu.Unlock()
	if timestamp.IsZero() {
...
func (h *StreamHub) BroadcastSessionUserMessageAt(
	...
	pendingUser := h.SetPendingUserAt(rootID, sessionKey, sessionName, agentName, model, modelDisplayName, mode, effort, fastService, planMode, content, timestamp)
