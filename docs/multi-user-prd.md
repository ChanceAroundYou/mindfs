...
> 状态：已定稿 2026-09-17 · 前置：单密码页面闸门 `8b59720`
...
| 期 | 内容 | 可独立验证 |
|---|---|---|
| P1 | `users.json` + bcrypt + 登录带用户名 + 账户 CRUD API + 登录页加用户名 | admin 可登录；建号→新号能登录；末位 admin 删不掉 |
| P2 | 每用户 config 目录 + `WorkspaceManager` + 构造函数传参 + meta 分账户 + `appPath` 注入 `?user=` | A/B 各自加目录互不可见；同一目录不串会话 |
| P3 | WS 连接归属 + 广播隔离 | B 收不到 A 的流式事件 |
| P4 | 用户管理界面 + 改自己密码 | admin 建号 → 新号首登见空列表 |
| P5 | 存量迁移（现有项目/会话归 admin） | admin 登录后看到原有全部项目与会话 |
